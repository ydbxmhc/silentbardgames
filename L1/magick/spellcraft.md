...
Simple, common spells are generally DL0. The ritual of binding for the spell creates potential effect, but unless they immediately release it they must decide how many points of the generated effect they want bound; they must bind all those as well, but at least one actual Role rank must always be included, and either a Role or Maneuver rank for every choice they keep.

All points bound into a spell are always available for the casting or countering of that spell, but they are unavailable for anything else. This is no loss for Maneuver ranks, as do any bound Maneuver ranks for countering. It's a direct opposition roll to suppress the targeted effect, or can be a free action resistance roll if it doesn't help anyone else. Binding a spell this way is more flexible. The only reason to bind more than one Role rank into a spell is for more effective fast-casting.

### Three Ways To Cast Spells

In some worlds only one of these is viable. Each way to do it can be the only way magic is cast, though the default is to use all three.

#### Full Ritual

This was already detailed above. In worlds where this is the only way to cast the spells are not cast in combat, but they can still be quite useful if your setting is designed for it.
...
There are two major downsides to this, however. The first is that you can't add any skill that wasn't already bound into it. You already bound it. It's already rolled when bound, and any unbound effect bleed away, wasted. The other is that when a spell is released the energy is spent, so it is no longer bound. It removes the spell from your repertoire until you can rebind it.

If this is the only way to cast in some worlds, bind some with more, even at the cost of a wider selection. This is a great way to sell them to other mages, but if you're planning to sell them you should remember that you have ranks of your Roles in those bottles...

---

### Recasting

Once a spell is bound the mage can channel energy through it in an attempt to replicate its effect in real time. This means the same rules apply, as if she were performing any other task. All unbound Role ranks apply, as do any bound in the spell, and all the Maneuver ranks are bound or not. If the spell is successfully cloned this way all bound ranks are available again for other uses. Going over or rolling a zero always destabilizes the spell, and it is lost until it can be rebound. The good news is that any Role rank bound in a zero always destabilizes the spell, and it is lost until it can be rebound.

Recasting is fast enough to do in a fast action. The character must be able to gesture broadly, and strongly invoke key words of power. If that attracts unwanted attention, it's the cost of keeping your spell bound.
...
The GM may tighten or broaden the applicability of spells to tune the game: if any fire spell allows countering any fire spell on the attacker's roll, the Spellweaver will be considerably more powerful; if the Firebolt spell only counts against another Firebolt spell, he will still be able to use all his talent. He will eventually learn the ability to apply certain spells more broadly: *Fire Counter* is a perfectly valid Maneuver.
...
A spell is a very complicated and particular skill that Spellweavers learn. Until you learn to do it for yourself, you'll never really understand it: that's no different from calculus, or alchemy, or a good rpaste. You start by following the formula, and eventually you start to get it.

Structurally, a spell is a construct in the Borderlands, a bit of clockwork engineering made entirely out of energy, wonder and will, with specific symbolic correspondences based on both personal and widely common concepts, usually represented in idioms, axioms, and similar sayings, that cause harmonic transference into the physical world when triggered through a suitable medium.

What is what you can do with it.
