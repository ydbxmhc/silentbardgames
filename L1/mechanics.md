...
Rolling is simple. Effective Level, or EL, determines how generally competent you are *for a particular task*, and all modifiers tweak that number. When the GM asks for a test, always start with your base Level. Add as many modifiers as make sense. Would a [Role](roles.html) apply? Would another *also* apply? [Maneuvers](maneuvers.html)? Active boosts? Situational bonuses? Add them all.

Is the character hurt? Blindfolded? Holding their breath against poison? Those modifiers *reduce* EL for that action. If your EL drops below 1, the task becomes impossible until you find ways to boost it back above zero. Negotiate disagreements. Once you establish the EL for a task, that's your target number. If it's something that *can't* be done without training, only add as much base Level as your rank in the relevant [Role](roles.html) - your best, if you have more than one.
...
EL can be radically different from task to task, for a lot of reasons. You might switch from using a [Role](roles.html) you're not very good at to one at which you excel, with well-developed [Maneuvers](maneuvers.html) to apply. Circumstances and [Boosts](actions-and-timing.html#boost-actions) can improve it, while Harm and opposing conditions drag it down.
